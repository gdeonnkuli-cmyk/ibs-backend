...
async function statsBailleur(bailleurId) {
  const r = await query(
    `SELECT
       ...
       (SELECT AVG(note) FROM avis WHERE bailleur_id = $1) AS note_moyenne,
       (SELECT COUNT(*) FROM avis WHERE bailleur_id = $1) AS avis_count`,
    [bailleurId]
  );
  const row = r.rows[0];
  ...
  return {
    offres_verifiees: offresVerifiees, baux_signes: bauxSignes, abonnes_count: Number(row.abonnes_count), tier,
    note_moyenne: row.note_moyenne ? Number(row.note_moyenne) : null, avis_count: Number(row.avis_count),
  };
}
