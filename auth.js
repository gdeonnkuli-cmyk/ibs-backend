...
// ── Mettre à jour son profil (locataire : profession, revenu, occupants — accompagne les candidatures) ──
router.patch("/me", requireAuth, async (req, res) => {
  try {
    const { profession, revenu_usd, nb_occupants, commune } = req.body;
    const r = await query(
      `UPDATE users SET
         profession = COALESCE($1, profession),
         revenu_usd = COALESCE($2, revenu_usd),
         nb_occupants = COALESCE($3, nb_occupants),
         commune = COALESCE($4, commune)
       WHERE id = $5 RETURNING *`,
      [profession ?? null, revenu_usd ? Number(revenu_usd) : null, nb_occupants ? Number(nb_occupants) : null, commune ?? null, req.user.id]
    );
    res.json({ user: publicUser(r.rows[0]) });
  } catch (e) { console.error(e); res.status(500).json({ error: "Erreur serveur." }); }
});
